...
if (
  ...
  // תיקון לבאג ידוע ב-Safari של אייפון: window.innerHeight משתנה כל הזמן
  // כשסרגל הכתובת מופיע/נעלם בזמן גלילה, מה שגורם לחישוב ה-progress
  // כאן להיות לא יציב ולתת לטקסט להישאר תקוע בשקיפות נמוכה (או להיראות
  // חופף בין הבלוקים). הפתרון: לשמור גובה יציב ולעדכן אותו רק כשבאמת
  // יש שינוי גודל אמיתי (למשל סיבוב מסך), לא בכל תזוזה של סרגל הכלים.
...
/* =========================================================
   BEFORE / AFTER SLIDERS (תומך בכמה סליידרים בעמוד אחד)
   ========================================================= */
...
function setupBeforeAfterSlider(slider) {
  ...
  function updateBeforeAfter(value) {
    ...
    const safeValue = Math.min(
      Math.max(
        Number.isFinite(numericValue) ? numericValue : 50,
        0
      ),
      100
    );

    beforeWrap.style.width = `${safeValue}%`;
...
document
  .querySelectorAll("[data-before-after]")
  .forEach(setupBeforeAfterSlider);
...
const QUESTIONNAIRE_DRAFT_KEY =
  "tlap_questionnaire_draft";
...
function saveQuestionnaireDraft() {
  ...
  try {
    ...
    console.warn(
      "לא ניתן היה לשמור טיוטה של השאלון:",
      error
    );
  }
}

const saveQuestionnaireDraftDebounced =
  debounce(
    saveQuestionnaireDraft,
    400
  );

function readQuestionnaireDraft() {
  try {
    const raw =
    ...
    console.warn(
      "לא ניתן היה לקרוא טיוטה שמורה של השאלון:",
      error
    );
...
function clearQuestionnaireDraft() {
  try {
    ...
    console.warn(
      "לא ניתן היה למחוק את הטיוטה השמורה:",
      error
    );
  }
}
...
function questionnaireHasProgress() {
  if (!questionnaire) {
    return false;
  }
...
let applicationExitConfirmModal = null;
let applicationExitConfirmLastFocus = null;
let applicationExitConfirmHideTimer = 0;

function createApplicationExitConfirmModal() {
  if (applicationExitConfirmModal) {
    return applicationExitConfirmModal;
  }
  ...
  modal.innerHTML = `
    <div
      class="application-exit-confirm__backdrop"
      data-exit-confirm-cancel
    ></div>

    <section
      class="application-exit-confirm__dialog"
      role="dialog"
      aria-modal="true"
      aria-labelledby="applicationExitConfirmTitle"
      aria-describedby="applicationExitConfirmDescription"
    >
      <div
        class="application-exit-confirm__icon"
        aria-hidden="true"
      >
        !
      </div>

      <h3 id="applicationExitConfirmTitle">
        לצאת מהשאלון?
      </h3>

      <p id="applicationExitConfirmDescription">
        כבר מילאת פרטים בשאלון.
        אם תצא עכשיו, התשובות יישמרו כטיוטה
        ותוכל להמשיך מאותה נקודה בהמשך.
      </p>

      <div
        class="application-exit-confirm__actions"
      >
        <button
          type="button"
          class="
  ...
  applicationExitConfirmModal = modal;

  return modal;
}

function isApplicationExitConfirmOpen() {
  return Boolean(
    applicationExitConfirmModal
      ?.classList.contains("open")
  );
}

function openApplicationExitConfirm() {
  const modal =
    createApplicationExitConfirmModal();

  window.clearTimeout(
    applicationExitConfirmHideTimer
  );

  applicationExitConfirmLastFocus =
    document.activeElement;
...
function closeApplicationExitConfirm(
  { restoreFocus = true } = {}
) {
  if (!applicationExitConfirmModal) {
    return;
  ...
  applicationExitConfirmHideTimer =
    window.setTimeout(
      () => {
        if (
          !modalToHide.classList.contains(
            "open"
          )
        ) {
...
function requestCloseApplicationModal() {
  if (isApplicationExitConfirmOpen()) {
    return;
  }

  if (questionnaireHasProgress()) {
    openApplicationExitConfirm();
    return;
  }
    ...
    radio.addEventListener(
      ...
      () => {
        if (
          ...
          if (!shouldShow) {
            const detailsField =
              eatingDifficultyDetailsWrap.querySelector(
                "textarea"
              );
...
questionnaire?.addEventListener(
  "input",
  saveQuestionnaireDraftDebounced
);
...
/* =========================================================
   QUESTIONNAIRE — MOBILE KEYBOARD & A11Y ENHANCEMENTS
   ========================================================= */
...
if (questionnaire) {
  questionnaire
    ...
    .forEach((input) => {
      if (!input.enterKeyHint) {
        input.enterKeyHint = "next";
      }
    });

  const birthDateInput =
    questionnaire.querySelector(
...
/* =========================================================
   QUESTIONNAIRE — VISUAL DESIGN ENHANCEMENTS
   ========================================================= */

function syncChoiceFieldSelection(
  fieldset
) {
  fieldset
...
document
  .querySelectorAll(".choice-field")
  .forEach((fieldset) => {
    syncChoiceFieldSelection(
      fieldset
    );
